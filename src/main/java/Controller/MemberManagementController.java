...
public class MemberManagementController {
    ...
    private void handleUpdateUser() {
        ...
        if (selectedUser == null) {
            ...
            util.ErrorDialog.showError("No User Selected", "Please select a user to update.", (Stage) rootVBox.getScene().getWindow());
            return;
        }
    ...
    try {
        ...
        util.ErrorDialog.showSuccess("Update Successful", "User details have been successfully updated.", (Stage) rootVBox.getScene().getWindow());
    } catch (SQLException e) {
        util.ErrorDialog.showError("Database Error", e.getMessage(), (Stage) rootVBox.getScene().getWindow());
    } catch (Exception e) {
    ...
    private boolean validateInputFields() {
        ...
        if (tfUsername.getText().isEmpty() || tfPhone.getText().isEmpty() || tfEmail.getText().isEmpty() ) {
            util.ErrorDialog.showError("Invalid Input", "Please fill in all fields.", (Stage) rootVBox.getScene().getWindow());
            return false;
        }
    ...
    private void handleDeleteUser() {
        User selectedUser = tvMembers.getSelectionModel().getSelectedItem();
        if (selectedUser == null) {
            util.ErrorDialog.showError("No User Selected", "Please select a user to delete.", (Stage) rootVBox.getScene().getWindow());
            return;
        }

        // Confirm deletion
        Alert confirmAlert = new Alert(Alert.AlertType.CONFIRMATION, "Are you sure you want to delete the user: " + selectedUser.getFullName() + "?", ButtonType.OK, ButtonType.CANCEL);
